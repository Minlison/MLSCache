...
static const NSUInteger kMLSYYMaxErrorRetryCount = 8;
static const NSTimeInterval kMLSYYMinRetryTimeInterval = 2.0;
static const int kMLSYYPathLengthMax = PATH_MAX - 64;
static NSString *const kMLSYYDBFileName = @"manifest.sqlite";
static NSString *const kMLSYYDBShmFileName = @"manifest.sqlite-shm";
static NSString *const kMLSYYDBWalFileName = @"manifest.sqlite-wal";
static NSString *const kMLSYYDataDirectoryName = @"data";
static NSString *const kMLSYYTrashDirectoryName = @"trash";
...
static UIApplication *_MLSYYSharedApplication() {
...
@implementation MLSYYKVStorageItem
@end

@implementation MLSYYKVStorage {
...
- (BOOL)_dbCheck {
    if (!_db) {
        if (_dbOpenErrorCount < kMLSYYMaxErrorRetryCount &&
            CACurrentMediaTime() - _dbLastOpenErrorTime > kMLSYYMinRetryTimeInterval) {
...
- (MLSYYKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    MLSYYKVStorageItem *item = [MLSYYKVStorageItem new];
...
- (MLSYYKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    MLSYYKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            MLSYYKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
 ...
 */
- (void)_reset {
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:kMLSYYDBFileName] error:nil];
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:kMLSYYDBShmFileName] error:nil];
    [[NSFileManager defaultManager] removeItemAtPath:[_path stringByAppendingPathComponent:kMLSYYDBWalFileName] error:nil];
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"MLSYYKVStorage init error" reason:@"Please use the designated initializer and pass the 'path' and 'type'." userInfo:nil];
    return [self initWithPath:@"" type:MLSYYKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(MLSYYKVStorageType)type {
    if (path.length == 0 || path.length > kMLSYYPathLengthMax) {
        NSLog(@"MLSYYKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > MLSYYKVStorageTypeMixed) {
        NSLog(@"MLSYYKVStorage init error: invalid type: %lu.", (unsigned long)type);
        return nil;
    }
    ...
    _path = path.copy;
    _type = type;
    _dataPath = [path stringByAppendingPathComponent:kMLSYYDataDirectoryName];
    _trashPath = [path stringByAppendingPathComponent:kMLSYYTrashDirectoryName];
    _trashQueue = dispatch_queue_create("com.ibireme.cache.disk.trash", DISPATCH_QUEUE_SERIAL);
    _dbPath = [path stringByAppendingPathComponent:kMLSYYDBFileName];
    ...
    if (![[NSFileManager defaultManager] createDirectoryAtPath:path
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error] ||
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kMLSYYDataDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error] ||
        ![[NSFileManager defaultManager] createDirectoryAtPath:[path stringByAppendingPathComponent:kMLSYYTrashDirectoryName]
                                   withIntermediateDirectories:YES
                                                    attributes:nil
                                                         error:&error]) {
        NSLog(@"MLSYYKVStorage init error:%@", error);
        return nil;
    }
    ...
    if (![self _dbOpen] || ![self _dbInitialize]) {
        ...
        if (![self _dbOpen] || ![self _dbInitialize]) {
            [self _dbClose];
            NSLog(@"MLSYYKVStorage init error: fail to open sqlite db.");
            return nil;
        }
    }
...
- (void)dealloc {
    UIBackgroundTaskIdentifier taskID = [_MLSYYSharedApplication() beginBackgroundTaskWithExpirationHandler:^{}];
    [self _dbClose];
    if (taskID != UIBackgroundTaskInvalid) {
        [_MLSYYSharedApplication() endBackgroundTask:taskID];
    }
}

- (BOOL)saveItem:(MLSYYKVStorageItem *)item {
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == MLSYYKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != MLSYYKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case MLSYYKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case MLSYYKVStorageTypeFile:
        case MLSYYKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (MLSYYKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (MLSYYKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    MLSYYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (MLSYYKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    MLSYYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
...
- (NSArray *)getItemForKeys:(NSArray *)keys {
    ...
    if (_type != MLSYYKVStorageTypeSQLite) {
        for (NSInteger i = 0, max = items.count; i < max; i++) {
            MLSYYKVStorageItem *item = items[i];
            if (item.filename) {
                item.value = [self _fileReadWithName:item.filename];
                if (!item.value) {
