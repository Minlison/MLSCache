...
/**
 MLSYYKVStorageItem is used by `MLSYYKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface MLSYYKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `MLSYYKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use MLSYYKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use MLSYYKVStorageTypeFile to get better performance.
 * You can use MLSYYKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, MLSYYKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    MLSYYKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    MLSYYKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    MLSYYKVStorageTypeMixed = 2,
};



/**
 MLSYYKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for MLSYYKVStorage is `initWithPath:type:`. 
 ...
 */
@interface MLSYYKVStorage : NSObject
...
@property (nonatomic, readonly) MLSYYKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(MLSYYKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is MLSYYKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is MLSYYKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is MLSYYKVStorageTypeMixed, then the item.value will be saved to file 
...
/**
 ...
 MLSYYKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is MLSYYKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is MLSYYKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is MLSYYKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable MLSYYKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable MLSYYKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `MLSYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<MLSYYKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `MLSYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<MLSYYKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
